...
function snap(value: number): number {
  return Math.round(value / precision.value) * precision.value
}

function clampUv(uv: StudioUvRect): StudioUvRect {
  const min = precision.value
  const width = Math.min(textureWidth.value, Math.max(min, snap(uv.width)))
  const height = Math.min(textureHeight.value, Math.max(min, snap(uv.height)))
  return {
    ...uv,
    width,
    height,
    x: Math.min(textureWidth.value - width, Math.max(0, snap(uv.x))),
    y: Math.min(textureHeight.value - height, Math.max(0, snap(uv.y))),
  }
}
...
function styleFor(binding: StudioTextureBinding): Record<string, string> {
  ...
  return {
    ...
    transform: `rotate(${uv.rotation}deg) scaleX(${uv.flipHorizontal ? -1 : 1}) scaleY(${uv.flipVertical ? -1 : 1})`,
  }
}
...
function startGesture(event: PointerEvent, binding: StudioTextureBinding, mode: 'move' | 'resize'): void {
  if (props.disabled || !stage.value) return
...
function finishGesture(event: PointerEvent): void {
  ...
  if (binding) publish(binding, binding.uv, true)
}
  ...
  <section class="uv-workspace">
    <header class="uv-toolbar">
      <div class="uv-summary">
        <strong>UV 2.0 · {{ face.toUpperCase() }}</strong>
    ...
    <div class="uv-actions" aria-label="UV tools">
      <button type="button" :disabled="disabled" class="primary" @click="emit('boxUv')">Box UV</button>
      ...
      <button type="button" :disabled="!activeBinding" @click="copyActive">Copy</button>
      <button type="button" :disabled="!activeBinding || !clipboard" @click="pasteActive">Paste</button>
      <button type="button" :disabled="!activeBinding" @click="resetActive">Reset</button>
      <button type="button" :disabled="!activeBinding" @click="fitTexture">Fit</button>
    </div>
    ...
    <footer class="uv-hint">
      <span>{{ precision }} px precision · {{ selectedFaceSet.size }} face{{ selectedFaceSet.size === 1 ? '' : 's' }} selected</span>
      <span>Tap an island · drag to move · corner to resize</span>
    </footer>
  </section>
</template>
...
.uv-island { position: absolute; z-index: 2; display: grid; place-items: center; box-sizing: border-box; border: 1px solid #90a198; background: #aab9b018; transform-origin: center; touch-action: none; }.uv-island.selected { border-color: #75b98a; background: #67ba7e25; }.uv-island.active { z-index: 4; border: 2px solid #63df8b; background: #63df8b30; box-shadow: 0 0 0 1px #07110a, 0 0 18px #3ddb7040; }
