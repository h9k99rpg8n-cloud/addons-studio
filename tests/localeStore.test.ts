...
describe('English and Spanish localization', () => {
  ...
  it('uses English as the source language and switches user-facing strings to Spanish', () => {
    ...
    expect(locale.t('Texture Core Stabilized')).toBe('Núcleo de texturas estabilizado')
    expect(locale.t('Unsaved UV')).toBe('UV sin guardar')
    expect(locale.t('Replace Color')).toBe('Reemplazar color')
    expect(locale.t('Build addons. Keep your workflow.')).toBe('Crea addons. Conserva tu flujo de trabajo.')
    expect(locale.t('Edit in Blockbench')).toBe('Editar en Blockbench')
    expect(locale.t('Collision Box')).toBe('Caja de colisión')
